...
For speed up the import, the disabled transactional features of PostgreSQL have been disabled. \footnote{\url{http://www.databasesoup.com/2015/02/running-with-scissors-mode.html}}.
...
For each table a GiST\footnote{\url{http://www.postgresql.org/docs/9.3/static/gist-intro.html}} index on the geometry
is created and a clustered geohashed index of the extens of a geometry\footnote{\url{http://postgis.net/docs/Box2D.html}} to make lookups that check if a geometry is in a certain tile faster. Especially the clustered index hash helped in speeding up the queries.
